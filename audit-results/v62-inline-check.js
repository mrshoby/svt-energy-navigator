      ...
      box.innerHTML = `
        <div class="q6-form">
          <div>
            ...
            <p>Preluăm automat curba de consum, tariful, surplusul PV și rezultatele anterioare. Completează doar procesele care pot fi mutate tehnologic.</p>
          </div>
    ...
    function buildQ6HourlySignals(){
      const ds = (() => { try{return JSON.parse(sessionStorage.getItem('svtDataset') || '{}')}catch(e){return {}} })();
      const rows = ds.rows || [];
      const buckets = Array.from({length:24}, (_,h)=>({hour:h, consumption:0, surplus:0, count:0, pv:0}));
      for(const r of rows){
        const d = new Date(r.timestamp);
        const h = Number(r.hour ?? d.getHours());
      ...
      return buckets.map(b => {
        ...
        return {hour:b.hour, avgConsumption:b.consumption / c, avgPv:b.pv / c, avgSurplus:b.surplus / c, solarScore:Math.max(0, solarHour)};
      });
    }
