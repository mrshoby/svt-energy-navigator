    ...
    function render(){
      ...
      grid.innerHTML = questions.map((q, idx) => {
        ...
        const isAvailable = mode === 'electric' && analysis?.chartRows?.length && (q.id === 1 || q.id === 2 || q.id === 3 || q.id === 4 || q.id === 5);
        const needsData = mode === 'electric' && analysis?.chartRows?.length && ((q.id === 2 && !hasPvData) || (q.id === 3 && !q3Stored) || (q.id === 4 && !q4Stored) || (q.id === 5 && !q5Stored));
        ...
        return `
          <div class="q-item" data-q-item="${q.id}">
            <button class="q-card ${q.title.length > 92 ? 'is-xlong' : q.title.length > 64 ? 'is-long' : ''} ${needsData ? 'needs-data' : isActive ? 'available' : 'soon'}" type="button" data-q="${q.id}" ${isActive ? '' : 'disabled'}>
              ...
              ${needsData ? `<span class="badge incomplete">${q.id === 5 ? 'Date incomplete · estimează producția suplimentară' : 'Date incomplete · completează datele necesare'}</span>` : isActive ? '<span class="answer-slot" title="Aici apare bifa după răspuns">✓</span>' : '<span class="badge">În curând</span>'}
            </button>
    ...
    function calculateQ4Potential(box, card){
      try{
        ...
        const usableArea = surfaceM2 * usablePercent / 100;
        const additionalKwp = usableArea / density;
        const netSpecificYield = specificYieldGross * Math.max(0.45, 1 - losses/100);
        const annualKwh = additionalKwp * netSpecificYield;
        ...
        const payload = {
          inputs:{lat,lon,surfaceM2,usablePercent,density,tilt,azimuth,losses,specificYieldGross,netSpecificYield,existingKwp,exportLimitKw,zeroExport,mounting},
      ...
      box.innerHTML = `
        <div class="q5-form">
          ...
          <div class="q5-actions">
            <span class="q5-note">Calculul separă energia utilizată direct, energia ce poate fi valorificată prin consum flexibil/BESS și surplusul exportat sau limitat.</span>
    ...
    function renderQuestion5Result(box, data){
      ...
      box.innerHTML = `
        <div class="q5-result-meta">
          <span class="q5-chip">PV suplimentar: ${fmt((data.source?.q4AdditionalKwp || 0),1)} kWp</span>
          <span class="q5-chip">Tarif consum: ${fmt(data.inputs?.tariffConsumption || 0,3)} lei/kWh</span>
          <span class="q5-chip">Export: ${data.inputs?.zeroExport === 'yes' ? 'zero-export' : fmt(data.inputs?.exportTariff || 0,3) + ' lei/kWh'}</span>
        </div>

        <div class="answer-summary">
          <div class="summary-card"><strong>${fmt(t.totalAdd/1000,1)} MWh/an</strong><span>producție suplimentară estimată</span></div>
          <div class="summary-card"><strong>${fmt(t.usefulTotal/1000,1)} MWh/an</strong><span>energie utilizabilă local direct/flexibil/BESS</span></div>
          <div class="summary-card"><strong>${fmt(t.usefulPercent,1)}%</strong><span>procent utilizabil local</span></div>
        </div>
        ...
        <div class="top-list">
          <div class="top-row">
            <div class="rank">1</div>
            <div><strong>Utilizare directă locală</strong><small>PV suplimentar consumat simultan cu consumul disponibil</small></div>
            <div class="cost" style="color:#067436">${fmt(t.direct/1000,1)} MWh</div>
          </div>
          <div class="top-row">
            <div class="rank">2</div>
            <div><strong>Flexibilitate / mutare consum</strong><small>energie suplimentară absorbită prin procese mutabile</small></div>
            <div class="cost" style="color:#067436">${fmt(t.flexUseful/1000,1)} MWh</div>
          </div>
          <div class="top-row">
            <div class="rank">3</div>
            <div><strong>BESS / stocare</strong><small>surplus stocat și consumat ulterior, cu randamentul introdus</small></div>
            <div class="cost" style="color:#067436">${fmt(t.bessUseful/1000,1)} MWh</div>
          </div>
          <div class="top-row">
            <div class="rank">4</div>
            <div><strong>Export / surplus rămas</strong><small>energie care nu este utilizată local direct, flexibil sau prin BESS</small></div>
            <div class="cost" style="color:#b45309">${fmt((t.exportKwh+t.curtailedKwh)/1000,1)} MWh</div>
          </div>
          <div class="top-row">
            <div class="rank">5</div>
            <div><strong>Valoare economică estimată</strong><small>valoare locală + valoare export; oportunitate pierdută calculată separat</small></div>
            <div class="cost">${fmt(t.localValue + t.exportValue,0)} lei/an</div>
          </div>
        </div>

        ${renderQ5DayBoard(data.intervalRows || [])}
        </div>
      `;
    }
